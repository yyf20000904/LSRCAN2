...
for j in range(500,1000,10):
    masks_list = [] 
    checkpoint ='experiment/our/checkpoint_RCANedsrset5_x2/epoch_'+str(j)+'.pth'
    model = rcanedsr.RCAN(upscale = opt.upscale_factor)#
    ...
    #print(opt.test_lr_folder + '/'+imname.split('/')[-1].split('.')[0] + ext)
        im_l = cv2.imread(opt.test_lr_folder + '/'+imname.split('/')[-1].split('.')[0] + ext)[:, :, [2, 1, 0]]   # BGR to RGB
        ...
        image = cv2.imread(output_folder, cv2.IMREAD_GRAYSCALE)
        masks_list.append(image) # Assuming lr_tensor contains the masks
        i += 1

    as_value = utils.get_AS2(masks_list)


    print("i:{},Mean PSNR: {}, SSIM: {},as:{}, TIME: {} ms".format(j,np.mean(psnr_list), np.mean(ssim_list), as_value,np.mean(time_list)))
